...
const getPreviousMonthSnapshot = async (): Promise<any> => {
  try {
    ...
    if (error) {
      console.warn('Error fetching previous snapshot:', error);
      return null;
    }
    return data;
  } catch (err) {
    console.warn('Failed to fetch previous month snapshot:', err);
    return null;
  }
};

const calculateGrowthRate = (current: number, previous: number | null): number => {
...
export const updateAnalyticsFromCurrentData = async () => {
  const registeredUsers = JSON.parse(localStorage.getItem('mindcare_registered_users') || '[]');
  const therapistServices = JSON.parse(localStorage.getItem('mindcare_therapist_services') || '[]');
  const bookings = JSON.parse(localStorage.getItem('mindcare_bookings') || '[]');
  ...
  if (previousSnapshot) {
    analytics.users.userGrowthRate = calculateGrowthRate(analytics.users.totalUsers, previousSnapshot.total_users);
    analytics.revenue.revenueGrowthRate = calculateGrowthRate(analytics.revenue.totalRevenue, previousSnapshot.total_revenue);
  } else {
    analytics.users.userGrowthRate = 0;
    analytics.revenue.revenueGrowthRate = 0;
  }
...
export const saveMonthlyAnalyticsSnapshot = async (analyticsData: PlatformAnalytics) => {
  try {
    ...
    const snapshot = {
      ...
      total_users: analyticsData.users.totalUsers,
      active_users: analyticsData.users.activeUsers,
      new_users_this_month: analyticsData.users.newUsersThisMonth,
      total_revenue: analyticsData.revenue.totalRevenue,
      monthly_revenue: analyticsData.revenue.monthlyRevenue,
      total_sessions: analyticsData.sessions.totalSessions,
      monthly_sessions: analyticsData.sessions.completedSessions,
      active_therapists: analyticsData.therapists.activeTherapists,
      total_therapists: analyticsData.therapists.totalTherapists
    };
    ...
    if (error) {
      console.warn('Error saving analytics snapshot:', error);
    }
  } catch (err) {
    console.warn('Failed to save monthly analytics snapshot:', err);
  }
};